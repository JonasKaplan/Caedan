...
impl Program {
    pub fn from_source(source_path: &Path) -> Result<Program, ParseError> {
        ...
        for procedure in result.procedures.into_iter() {
            procedures.insert(procedure.name.clone(), Procedure::new(&procedure.name, procedure.instructions));
        }
    ...
    pub fn run(self) -> () {
        ...
        while !call_stack.is_empty() {
            ...
            if let Some(call) = procedure.execute(region, frame.pointer, &self.regions) {
